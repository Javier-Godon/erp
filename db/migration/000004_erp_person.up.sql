...
--***********REQUEST*******************
CREATE TABLE IF NOT EXISTS PERSON.request
(
    request_id       varchar(256) PRIMARY KEY,
    workflow         varchar(256),
    reason_id        integer,
    id_proof_type_id integer,
...
(
    person_id                             varchar(256) PRIMARY KEY,
    is_company                            boolean,
    ...
    id_address_birth                      varchar(256),
    id_current_address                    varchar(256)
);

CREATE TABLE IF NOT EXISTS PERSON.person_task
(
    person_task_id          varchar(256),
    request_id              varchar(256),
    person_id               varchar(256),
    message_id              varchar(256),
    orchestrator_process_id varchar(256),
...
CREATE TABLE IF NOT EXISTS PERSON.personal_links
(
    personal_link_id varchar(256) PRIMARY KEY,
    main_person_id   varchar(256),
    linked_person_id varchar(256)
);

--************COUNTRIES_LIST**************
CREATE TABLE IF NOT EXISTS PERSON.countries
(
    country_id   varchar(256) primary key,
...
INSERT INTO PERSON.countries (country_id, english_name, spanish_name, alpha_2, alpha_3)
VALUES (4, '', 'Afganistán', 'af', 'afg'),
       (8, '', 'Albania', 'al', 'alb'),
       (276, '', 'Alemania', 'de', 'deu'),
       (20, '', 'Andorra', 'ad', 'and'),
       (24, '', 'Angola', 'ao', 'ago'),
       ...
       (68, 'Bolivia (Plurinational State of)', 'Bolivia', 'bo', 'bol'),
       (70, '', 'Bosnia y Herzegovina', 'ba', 'bih'),
       (72, '', 'Botsuana', 'bw', 'bwa'),
       (76, 'Brazil', 'Brasil', 'br', 'bra'),
       (96, '', 'Brunéi', 'bn', 'brn'),
       (100, '', 'Bulgaria', 'bg', 'bgr'),
       ...
       (826, 'United Kingdom', 'Reino Unido', 'gb', 'gbr'),
...
CREATE OR REPLACE FUNCTION public.search_person_as_you_type(search_string text)
